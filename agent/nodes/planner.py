...
from agent.models.llm_models import PlannerOutput
...
def planner(state: B2BGraphState):
    out = planner_model.invoke(
        [
            f""" You are a planner for a B2B e-commerce platform. You need to break down the user's 
    request into a series of steps. Use the following rules to break down the request:
    1. If the user is looking to do any cart operation, use the CART_MANAGER plan. But if you dont know price for the item, first use ACME_AGENT AND STARK_AGENT 
    plan to get item details AND then use the CART_MANAGER plan.
    2. If the user is looking to do item lookup or order operation, use the ACME_AGENT AND STARK_AGENT rule.
    ...
    return {"plans": out.plans, "items": [], "order_numbers": []}
